...
api.request = (endpoint, method, settings, body) => {
  ...
  return fetch(url, api.init(method, settings, body)).then(api.checkStatus).then(fetchDebugHandler(config, method, settings, body, url))
}
...
api.checkStatus = stream => {
  ...
  if (stream.ok) {
    if (status === 204) {
      // 204 has no body, resulting in a JSON parsing error
      return ''
    }
  }
